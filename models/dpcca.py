"""=============================================================================
Full DPCCA model for images and associated high-dimensional signal.
============================================================================="""
...
class DPCCA(nn.Module):
    ...
    def __init__(self, cfg, latent_dim, em_iters=1, use_gene_net=True):
        ...
        self.use_gene_net = use_gene_net
        self.latent_dim   = latent_dim

        if use_gene_net:
            dims = [cfg.IMG_EMBED_DIM, cfg.GENE_EMBED_DIM]
        else:
            dims = [cfg.IMG_EMBED_DIM, cfg.N_GENES]
...
# ------------------------------------------------------------------------------
    ...
    def estimate_z_given_x(self, x, threshold):
        """Estimate the latent variable z given our data x.
        """
        y = self.encode(x)
        return self.pcca.estimate_z_given_y(y, threshold).t()
